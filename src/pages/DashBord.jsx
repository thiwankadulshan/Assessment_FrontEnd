...
import { Increment, Decrement } from "../sheard/Pgination";
...
const DashBord = () => {
    ...
    const chageState = (code, state) => {
        postRequest(apiEndPoint.editItemState, { itemCode: code, state: state })
            .then((response) => response.status === "success" ? navigate("/dashbord") : navigate("/dashbord"))
            .catch((error) => console.error(error))
    }
    ...
    return (
        <>
                    ...
                    <tbody className="tbody">
                        {data.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td><button type="button" className="btn btn-secondary" onClick={() => genarateDocument(item.itemCode)}>Document</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div className="pagination-data-buttons">
                {number2 > 1 ? <ul className="pagination-previous text-dark" onClick={() => Decrement(setNumber, number)}>Previest</ul> : ""}
